...
router.post('/message', async (req, res) => {
    try {
        ...
        if (!senderId || !message) {
            return res.status(400).json({ message: "Please fill all the required fields (senderId, message)" });
        }
        ...
        if (!conversationId && receiverId) {
            const newConversation = new conversationModal({
                members: [senderId, receiverId, { senderName: senderName.fullName }, { receiverName: receiverName.fullName }],
            });
            await newConversation.save();

            const newMessage = new messageModal({ conversationId: newConversation._id, senderId, message });
            ...
            const newMessage = new messageModal({ conversationId, senderId, message, receiverId, img: receiverName.image_Id });
            await newMessage.save();

            return res.status(201).json({
                message: "Message sent successfully",
                data: { conversationId, messageId: newMessage._id, senderUserName: senderName.fullName, receiverUserName: receiverName.fullName, img: receiverName.image_Id }
            });
        } else {
            return res.status(400).json({ message: "Conversation ID or Receiver ID is required" });
        }
    } catch (error) {
