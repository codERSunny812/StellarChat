...
router.post('/conversation', async (req, res) => {
    try {
        ...
        res.status(201).json({
            ...
            data: { newConversation, fullName: receiverName.fullName, img: receiverName.image_Id }
        });
    } catch (error) {
        res.status(500).json({ message: `Error in creating conversation: ${error}` });
    }
});
...
router.get('/conversation/:userId', async (req, res) => {
    try {
        ...
        const conversationUserData = await Promise.all(conversations.map(async (conversation) => {
            ...
            return {
                user: { receiverId: user._id, email: user.email, fullName: user.fullName, img: user.image_Id },
        ...
        res.status(500).json({ message: `Error in fetching conversations: ${error}` });
    }
});
